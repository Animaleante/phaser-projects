...
TheGame.prototype = {
    ...
    create: function() {
        ...
        this.placeMarker(gameOptions.playerCol, gameOptions.playerRow);
    },
    update: function() {
        if(this.playerMove) {
            if(this.cursors.left.isDown && this.cursors.right.isUp) {
                gameOptions.playerCol -= (1 - gameOptions.playerRow % 2);
                gameOptions.playerRow++;
                this.placeMarker(gameOptions.playerCol, gameOptions.playerRow);
                this.playerMove = false;
            } else if(this.cursors.right.isDown && this.cursors.left.isUp) {
                gameOptions.playerCol += (gameOptions.playerRow % 2);
                gameOptions.playerRow++;
                this.placeMarker(gameOptions.playerCol, gameOptions.playerRow);
                this.playerMove = false;
            }
    ...
    placeMarker: function(posX, posY) {
        if(posY > 6 || posX < 0 || posX > Math.floor(gameOptions.gridSizeX / 2) - posY % 2) {
            posX = 2;
            posY = 0;
            gameOptions.playerCol = 2;
            gameOptions.playerRow = 0;
        }
