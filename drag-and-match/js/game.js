...
TheGame.prototype = {
    preload: function() {
        // game.load.spritesheet("tiles", "assets/sprites/tiles.png", gameOptions.spritesheetSize, gameOptions.spritesheetSize);
        game.load.spritesheet("tiles", "assets/sprites/tiles2.png", gameOptions.spritesheetSize, gameOptions.spritesheetSize);
    ...
    handleStop: function() {
        ...
        switch(this.dragDirection) {
            case HORIZONTAL_DRAG:
                shiftAmount = Math.floor(this.distX / (gameOptions.tileSize / 2));
                ...
                offset = this.distX % gameOptions.tileSize;

                if(Math.abs(offset) > gameOptions.tileSize / 2) {
                    if(offset < 0) {
                        offset += gameOptions.tileSize;
                    } else {
                        offset -= gameOptions.tileSize;
                    }
                }
                ...
                break;
            case VERTICAL_DRAG:
                shiftAmount = Math.floor(this.distY / (gameOptions.tileSize / 2));
                shiftAmount = Math.ceil(shiftAmount / 2) % gameOptions.fieldSize;
                ...
                offset = this.distY % gameOptions.tileSize;
                if(Math.abs(offset) > gameOptions.tileSize / 2) {
                    if(offset < 0) {
                        offset += gameOptions.tileSize;
                    } else {
    ...
    fillVerticalHoles: function() {
        ...
        for (i = gameOptions.fieldSize - 2; i >= 0; i--) {
            for (j = 0; j < gameOptions.fieldSize; j++) {
